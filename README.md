...
Claude（Web版・Claude Code・Cursor 等）で以下のいずれかのプロンプトを使います。

#### パターンA: ファイル名まで決めて依頼する（標準）

`new-article.py` で先にファイルを作ってから依頼するパターンです。何を書くか明確に決まっている場合に向いています。
...
#### パターンB: 媒体・ジャンル選定からClaudeに任せる（曖昧依頼）

「とりあえず○○について書きたい」レベルの段階で依頼するパターンです。Claude が媒体・ジャンル・ファイル名を提案してから書き始めます。

```
このリポジトリの CLAUDE.md を読んで、
○○について記事を書きたい。

媒体・ジャンル・slugの選定から相談に乗って。
決まったら scripts/new-article.py を使ってファイルを生成し、本文を書いて。
```

CLAUDE.md の「依頼への対応フロー」に沿って、Claudeは以下のように動きます：

1. 媒体（Qiita / Zenn / note / はてな / Dev.to）と ジャンル（10種類のどれか）の選択肢を提示
2. 題材の深掘り質問（何を作ったか / 一番伝えたいことは何か など）
3. `new-article.py` でファイル生成
4. テンプレートに沿って本文を執筆

> **Note**: Web版Claudeなど、リポジトリのファイルを直接読めない環境では、`CLAUDE.md` の中身をプロンプトに貼り付けるか、リポジトリを Claude Code / Cursor で開いて使うのが確実です。
...
依頼の粒度に応じて、以下の3パターンを使い分けます。

#### パターン1: 曖昧依頼（媒体・ジャンル選定からClaudeに任せる）

「とりあえず○○について書きたい」という段階で使います。Claude が CLAUDE.md の「依頼への対応フロー」に沿って、媒体・ジャンル・slug を提案してくれます。
...
書く内容と媒体が決まっていて、`new-article.py` で先にファイルを作ってある場合に使います。
...
このリポジトリは `CLAUDE.md` パターンを採用しているため、Claude Code、Cursor、Continue.dev など、**プロジェクトルートの規約ファイルを参照するツール全般** と相性が良いです。これらのツールでは `CLAUDE.md` が自動で読み込まれるため、プロンプトに「CLAUDE.md を読んで」と毎回書く必要すらありません。

### Web版Claude / 通常のチャットで使う場合

Web版Claudeなどリポジトリのファイルを直接参照できない環境では、以下のいずれかが必要です：

- 依頼時に `CLAUDE.md` の中身をプロンプトに貼り付ける
- 関連するファイル（`CLAUDE.md`、対象のテンプレート、対象の draft ファイル）をアップロードして依頼する

確実なのは Claude Code / Cursor 経由での利用です。
